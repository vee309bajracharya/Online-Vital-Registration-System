...
const EditBirthCertificateForm = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            setIsLoading(true)
            try {
                await axiosClient.put(`/birth-certificate/edit/${id}`, values)
                ...
                if (response?.data?.details) {
                    ...
                    toast.error('Failed to update birth certificate. Please try again')
                }
            } finally {
                setIsLoading(false)
            }
    ...
    useEffect(() => {
        const fetchCertificate = async () => {
            try {
                const response = await axiosClient.get(`/birth-certificate/${id}`)
                const cert = response.data.data

                // Populate form with existing data
                formik.setValues({
                    full_name_en: cert.full_name_en,
                    full_name_np: cert.full_name_np,
                    gender: cert.gender,
                    dob_en: cert.dob_en,
                    dob_np: cert.dob_np,
                    birth_place_en: cert.birth_details.birth_place_en,
                    birth_place_np: cert.birth_details.birth_place_np,
                    grandfather_name_en: cert.grandfathers.grandfather_name_en,
                    grandfather_name_np: cert.grandfathers.grandfather_name_np,
                    parents: cert.parent_details.map(parent => ({
                        parent_type: parent.parent_type,
                        full_name_en: parent.full_name_en,
                        full_name_np: parent.full_name_np,
                        citizenship_number_en: parent.citizenship_number_en,
                        citizenship_number_np: parent.citizenship_number_np,
                    })),
                    province_en: cert.address.province_en,
                    province_np: cert.address.province_np,
                    district_en: cert.address.district_en,
                    district_np: cert.address.district_np,
                    municipality_en: cert.address.municipality_en,
                    municipality_np: cert.address.municipality_np,
                    ward_number: cert.address.ward_number,
                    informant_name_en: cert.registrations.informant_name_en,
                    informant_name_np: cert.registrations.informant_name_np,
                    citizenship_number_en: cert.registrations.citizenship_number_en,
                    citizenship_number_np: cert.registrations.citizenship_number_np,
                })
            } catch (error) {
                toast.error('Failed to load certificate data', error)
                navigate('/dashboard')
            } finally {
...
export default EditBirthCertificateForm